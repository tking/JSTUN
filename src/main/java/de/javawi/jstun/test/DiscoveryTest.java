...
public class DiscoveryTest {
	...
	private boolean test2() throws UtilityException, SocketException, UnknownHostException, IOException,
			MessageAttributeParsingException, MessageAttributeException, MessageHeaderParsingException {
		int timeSinceFirstTransmission = 0;
		int timeout = timeoutInitValue;
		while (true) {
			try {
				...
				try (DatagramSocket sendSocket = new DatagramSocket(
						new InetSocketAddress(sourceIaddress, sourcePort))) {
				...
				if (timeSinceFirstTransmission < 7900) {
					...
					if (!nodeNatted) {
						...
						// not is natted
						// redo test 1 with address and port as offered in the changed-address message
						// attribute
						return true;
					}
				}
	...
	private boolean test1Redo() throws UtilityException, SocketException, UnknownHostException, IOException,
			MessageAttributeParsingException, MessageHeaderParsingException {
		int timeSinceFirstTransmission = 0;
		int timeout = timeoutInitValue;
		while (true) {
			...
			try {
				...
				MappedAddress ma2 = (MappedAddress) receiveMH
	...
	private void test3() throws UtilityException, SocketException, UnknownHostException, IOException,
			MessageAttributeParsingException, MessageAttributeException, MessageHeaderParsingException {
		int timeSinceFirstTransmission = 0;
		int timeout = timeoutInitValue;
		while (true) {
